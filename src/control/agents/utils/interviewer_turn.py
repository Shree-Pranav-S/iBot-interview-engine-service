"""Helpers for staged interviewer-turn classification and live response handling."""
...
def _validate_live_response(
    ...
    if is_technical and result.answer_strength is None:
        raise QuestionValidationException("technical answer requires answer_strength")
    if not is_technical and result.answer_strength is not None:
        raise QuestionValidationException(
            "non-technical answer requires answer_strength=null"
        )
