"""Non-blocking transcript and violation persistence helpers."""
...
def schedule_bot_persistence(state: InterviewState) -> None:
    """Queue the pending bot turn for background persistence, if present."""
    pending = state.get("pending_bot_turn")
    if pending:
        _schedule(state, transcript_items=[pending])


def schedule_candidate_persistence(state: InterviewState) -> None:
    """Queue pending candidate speech and violations for background persistence."""
