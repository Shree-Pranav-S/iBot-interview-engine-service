...
class TranscriptEvidence(StrictEvaluationModel):
    ...
    skill: str | None = None
    question: str | None = Field(default=None, max_length=900)
...
class SkillScoreReport(StrictEvaluationModel):
    ...
    signals_demonstrated: list[str] = Field(default_factory=list, max_length=20)
    signals_missing: list[str] = Field(default_factory=list, max_length=20)
...
class SectionSummaryReport(StrictEvaluationModel):
    ...
    difficulty_reached: str | None = None
    questions_asked: int = Field(ge=0)
    evidence: list[TranscriptEvidence] = Field(default_factory=list, max_length=12)
    ...
    score_basis: str | None = Field(default=None, max_length=700)


class IntroSectionEvaluation(StrictEvaluationModel):
    section_summary: SectionSummaryReport
    strengths: list[str] = Field(default_factory=list, max_length=8)
    concerns: list[str] = Field(default_factory=list, max_length=8)


class BehaviouralCulturalEvaluation(StrictEvaluationModel):
    behavioural_score: float = Field(ge=0.0, le=100.0)
    behavioural_evidence: list[str] = Field(min_length=1, max_length=20)
    behavioural_summary: str = Field(min_length=1, max_length=2200)
    cultural_fit_score: float = Field(ge=0.0, le=100.0)
    cultural_fit_evidence: list[str] = Field(min_length=1, max_length=20)
    cultural_fit_summary: str = Field(min_length=1, max_length=2200)
    section_summary: SectionSummaryReport
...
class FinalSynthesisReport(StrictEvaluationModel):
    technical_dimension_score: float = Field(ge=0.0, le=100.0)
    score_evidence: list[str] = Field(min_length=1, max_length=24)
    score_summary: str = Field(min_length=1, max_length=2800)
    ...
    overall_narrative: str = Field(min_length=1, max_length=2800)
    strengths: list[str] = Field(min_length=1, max_length=16)
    concerns: list[str] = Field(min_length=1, max_length=16)
    ...
    recommendation_reasoning: str = Field(min_length=1, max_length=2800)
